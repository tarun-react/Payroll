...
class DepartmentSelector extends Component {
  ...
  fetchDepartments() {
    ...
    getDepartments(staffId)
      .then(resp => {
        ...
        let timeArr = outlet[0].time
        this.props.updateTimings(timeArr)
        this.setState({
  ...
  renderOutlets = ({item, index}) => {
    return (
      <TouchableWithoutFeedback
        onPress={() => this.selectOutlet(item.outletID, item.name,item)}>
        <View style={{padding: 5, paddingLeft: 13}}>
  ...
  selectOutlet(id, outlet,item) {
    HelperMethods.animateLayout();
    this.props.updateTimings(item.time)
