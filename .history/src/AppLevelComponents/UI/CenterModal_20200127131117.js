...
class CenterModal extends Component {
  ...
  componentWillMount() {
    ...
    this.getTimings(dateSelected);
  }

  getTimings(date,outletId) {
    getShiftTimings(date,outletId)
      .then(resp => {
        HelperMethods.animateLayout()
        this.setState({shiftTimings: resp.message})
        let firstTime = resp.message[0].shift_time;
        let firstId = resp.message[0].shiftID;
        shiftTimeId = firstId;
  ...
  renderItem = ({item, index}) => {
    return (
      <TouchableWithoutFeedback
        onPress={() => this.selectShift(item.shiftID, item.shift_time)}>
        <View style={{padding: 5, paddingLeft: 13}}>
          <CustomText
            size={14}
            text={item.shift_time}
                ...
                return (
                  <Modal
                    ...
                    <CustomText
                      ...
                      <DepartmentSelector updateTimings={outletId => this.getTimings(dateSelected,outletId)} staffId={currentContext.userData.loginuserID} depIdGetter={id => (depId = id)} />
